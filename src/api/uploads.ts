...
/** 0..1 업로드 진행률 콜백 (네이티브만; 웹 fetch 는 진행률 미지원). */
 ...
 */
function sendNative(form: FormData, onProgress?: UploadProgress): Promise<RawResponse> {
...
function send(form: FormData, onProgress?: UploadProgress): Promise<RawResponse> {
  return Platform.OS === "web" ? sendWeb(form) : sendNative(form, onProgress);
}

/**
 * 로컬 사진 URI(카메라/갤러리) → 서버에 업로드하고 공개 URL 배열을 돌려준다.
 * 목 모드에서는 로컬 URI 를 그대로 쓴다.
 * onProgress: 0..1 진행률 (네이티브만).
...
export async function uploadPhotos(
  ...
  let res: RawResponse;
  try {
    res = await send(await buildForm(localUris), onProgress);
  } catch (e) {
